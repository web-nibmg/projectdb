	...
	<body onload='load_body();'>
		...
		<script language='javascript' type='text/javascript'>
			...
			function ajax_search(){
				...
				$.post('/Data_Portal/Backend/Admin/PMID_search.php', {search_item : search_PMID}, function(data){
		...
		<script language='javascript' type='text/javascript'>
			...
			function edit_delete_ajax(){
				$.post('/Data_Portal/Backend/Admin/PMID_edit_delete.php', {request_type : $('#action_type').val(), PMID : $('#PMID_transit').val(), PMID_final : $('#PMID_final_transit').val()}, function(){
		...
		<script language='javascript' type='text/javascript'>
	    	...
	    	function edit_PMID_ajax(parent_id){
				...
				if($('input', '#' + parent_id).val() == 'Edit'){
					...
					temp_PMID = sanitize_input($('input', '#' + temp_ID + 0).val());
				...
				//if the button value is already "submit", it validates the user input and submits them to the server on confirmation...
					...
					temp_PMID_final = sanitize_input($('input', '#' + temp_ID + 0).val());
					if(temp_PMID_final == ''){
						alert('You Can Not Leave PMID Field Blank');
						return false;
					}
					if(confirm('Do You Really Want To Submit?')){
						$.post('/Data_Portal/Backend/Admin/PMID_edit_delete.php', {request_type : 'edit', PMID : $('#PMID_transit').val(), PMID_final : temp_PMID_final}, function(){
	    ...
	    <script language='javascript' type='text/javascript'>
	    	...
	    	function delete_PMID(parent_id){
				...
				if($('input', '#' + parent_id).val() == 'Delete'){
					...
					if(confirm('Do You Really Want To Submit?')){
						$.post('/Data_Portal/Backend/Admin/PMID_edit_delete.php', {request_type : 'delete', PMID : temp_PMID}, function(){
