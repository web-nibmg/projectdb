<?php
	...
	if(!isset($_POST['old_password']) || !isset($_POST['new_password']) || !isset($_POST['confirm_new_password'])){
		header('Location: ../App_Files/Change_Password.php?flag=2');
		exit();
	}
	//null check ends...
	...
	if($old_password != $str){ 
		header('Location: ../App_Files/Change_Password.php?flag=1');
		mysqli_close($cn);
		exit();
	}
	...
	//redirects to "Change_Password.php" if the query is failed to execute...
		...
		mysqli_close($cn);
		header('Location: ../App_Files/Change_Password.php?flag=4');
		exit();
	}
